...
class RPSGame():
    ...
    def run(self):
        ...
        self.action_menu_view = ActionMenuView(parent=self)
        self.message_popup_view = MessagePopupView(parent=self)
    ...
    def init_iotf(self):
        ...
        self.set_status_text('Waiting for network')
        if not util.wait_network():
            self.set_status_text('Cannot connect WiFi')
    ...
    def check_network(self):
        ...
        if not util.wait_network():
            self.set_status_text('Cannot connect WiFi')
            raise Exception('Cannot connect WiFi')
        self.set_status_text('Listing RPS games')
        self.close_status_box()

    # Events
    def list_games(self):
        self.open_status_box()
        self.set_status_text('Wait for list...')
    ...
    def rps_cb(self, obj):
        ...
        if cmd_type == 'games':
            self.appear_view(self.game_list_view)
            self.game_list_view.update(cmd_value['games'])
            self.close_status_box()
        elif cmd_type == 'join':
            self.appear_view(self.action_menu_view)
        elif cmd_type == 'chat':
