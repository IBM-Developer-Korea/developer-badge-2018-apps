...
미세먼지 센서는 Sharp 전자의 [GP2Y1014AU0F](https://www.devicemart.co.kr/goods/view?no=1327422) 제품을 이용합니다. 

GP2Y1014AU0F 센서는 PM2.5 까지 측정이 가능하며, 물리적인 공기 중 투과도를 측정하여 대기 중 먼지 밀도를 측정하는 센서입니다. 센서에 대한 상세 규격 정보는 아래와 같이 각 제품 공급 홈페이지에서 확인 할 수 있으니 참고 하기 바랍니다.
...
> ※ 참고 사항  
> Developer Day 2018 뱃지의 동작 전원은 3.3V 기준이나, GP2Y1014AU0F 5V입니다. 일반적인 경우라면 3.3-5V 레벨 변환 회로가 필요합니다. 그러나, 레벨 변환 회로 구성에 따른 전반적인 복잡도 증가 및 3.3V 환경에서도 어느정도 일정한 결과가 나오는 것을 확인했기에, Developer Day 2018 뱃지에는 별도의 레벨 변환 회로 없이 구성 되었습니다.
...
패키지의 콘덴서는 아래 그림과 같이 회색 표시가 된 부분을 보이도록 하여 남땜 합니다.
...
완성된 에제 코드는 아래 소스 코드를 참고하시기 바랍니다.

* [아두이노 예제 코드](dust.ino)

### 마이크로 파이썬 예제 코드

앞서 작성한 아두이노 에제 코드를 바탕으로 마이크로 파이썬으로 코드로 만들면 다음 에제 코드와 같습니다.
...
결과적으로 마이크로 파이썬에서 마이크로초 단위 통제가 안되는 현상으로 볼 수 있지만, 약간의 트릭으로 지연 시간을 조정하여 일정한 값이 출력되도록 할 수 있습니다. 이는 다음 에제를 참고 하기 바랍니다.

* [마이크로 파이썬 예제 코드 - 타이밍 수정본](dust_adjusted.py)

그 외에는 마이크로 파이썬이 포팅된 Developer Day 2018 뱃지의 펌웨어에 C 코드로 ADC 값을 읽는 코드를 작성하여 넣는 방법이 있는데 본 문서에서는 다루지 않겠습니다.
